...
describe('settingsValidatorFactory', () => {
  ...
  describe('Solr:', () => {
    ...
    var funkyDocs = [
      {
        ...
        text: 'For purposes of computing.',
        section: '9.1-703',
        structure: 'Commonwealth Public Safety/Overtime Compensation...',
        type: 'law',
        uid: '1234',
        _version_: 1512671587453632512,
      },
      {
        catch_line: 'Powers, duties...',
        text: 'foo',
        section: '45.1-361.28',
        structure: 'Mines and Mining/The Virginia Gas and Oil Act',
        tags: ['locality'],
    ...
    describe('Validate URL:', () => {
      ...
      it('throws when proxy is combined with Solr default JSONP transport', () => {
        ...
        }).toThrowError('It does not make sense to proxy a JSONP connection, use GET instead.');
      });
  ...
  describe('ES:', () => {
    ...
    beforeEach(() => {
      validator = new SettingsValidatorFactory(esSettings);
    });
  ...
  describe('SearchApi:', () => {
    ...
    beforeEach(() => {
      validator = new SettingsValidatorFactory(apiSettings);
    });

    describe('Generates candidate ids', () => {
      it('selects only ids occuring across all docs', async () => {
        mockBackend
          .expectGET(apiSettings.searchUrl + '?' + apiSettings.args)
          .respond(200, fullResponse);
        var called = 0;
        await validator.validateUrl().then(() => {
          expect(validator.idFields.length).toBe(3);
  ...
  describe('Algolia:', () => {
    ...
    beforeEach(() => {
      validator = new SettingsValidatorFactory(algoliaSettings);
    });
  ...
  describe('Vectara:', () => {
    ...
    beforeEach(() => {
      validator = new SettingsValidatorFactory(vectaraSettings);
    });
