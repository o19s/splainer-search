...
describe('explainSvc', () => {
  it('parses mockExplain', () => {
    explainSvc.createExplain(mockExplain);
  });

  it('returns null/empty explain', () => {
  ...
  it('how does it perform', () => {
    ...
    d = new Date();
    var stop = d.getTime();
    // Performance benchmark — keep silent in normal test output
  ...
  describe('elasticsearch explains', () => {
    it('deals with Math.minOf', () => {
      ...
      expect(Object.keys(matches.vecObj).length).toBe(1);
      expect(Object.keys(matches.vecObj)[0]).toContain('exp');
    });
