...
describe('searchSvc: Solr', () => {
  ...
  it('access solr with mock solr params using JSONP', async () => {
    ...
    mockBackend
      .expectJSONP(urlContainsParams(mockSolrUrl, expectedParams))
  ...
  it('access solr with mock solr params using GET', async () => {
    ...
    mockBackend.expectGET(urlContainsParams(mockSolrUrl, expectedParams)).respond(200, mockResults);
    await searcher.search();
    mockBackend.verifyNoOutstandingExpectation();
  });
  ...
  describe('highlights', () => {
    ...
    it('asks for highlights', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedHlParams))
    ...
    it('doesnt request hls if hls off', async () => {
      ...
      var copiedResp = structuredClone(fullSolrResp);
      mockBackend
        .expectJSONP(urlMissingParams(mockSolrUrl, expectedHlParams))
        .respond(200, copiedResp);
      var called = 0;
      await searcher.search().then(function () {
        called++;
  ...
  describe('explain info', () => {
    ...
    it('populates explain()', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedDebugParams))
    ...
    it('doesnt request debug info when configured not to', async () => {
      ...
      delete copiedResp.debug;
      mockBackend
        .expectJSONP(urlMissingParams(mockSolrUrl, expectedDebugParams))
        .respond(200, copiedResp);
      await searcher.search().then(function () {
        var solrDocs = searcher.docs;
        expect(solrDocs[0].explain('http://larkin.com/index/')).toBe(null);
  ...
  describe('alt id field tests', () => {
    ...
    it('escapes percents in the query', async () => {
      ...
      var searcher = searchSvc.createSearcher(
        fieldSpec,
        mockSolrUrl,
        mockSolrParamsWithMm,
        mockQueryText,
      );
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedParamsMm))
        .respond(200, mockResultsAltId);
      await searcher.search();
      mockBackend.verifyNoOutstandingExpectation();
    });
  ...
  describe('search', () => {
    it('passes the rows param and sets it to 10 by default', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedSearchParams))
        .respond(200, mockResults);
      var called = 0;
      await searcher.search().then(function () {
        called++;
    ...
    it('makes querydocs with document url with NO facet', async () => {
      ...
      await searcher.search().then(function () {
        ...
        solrDocs.forEach(function (doc) {
          expect(
            urlContainsParams(mockSolrUrl, expectedFacetField).test(doc._url('id', '12')),
          ).toBeFalsy();
    ...
    it('doesnt sanitize solr arguments when told not to', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, mockUncleanSolrParams))
        .respond(200, mockResults);
      await searcher.search();
      mockBackend.verifyNoOutstandingExpectation();
    });
    ...
    it('searches with fl == *', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, testSolrParams))
        .respond(200, mockResults);
      await searcher.search();
      mockBackend.verifyNoOutstandingExpectation();
    });
    ...
    it('escape special chars in queryText', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, thisExpectedParams))
        .respond(200, mockResults);
      await searcher.search();
      mockBackend.verifyNoOutstandingExpectation();
    });
          ...
          groups: [
            {
              ...
              doclist: {
                numFound: 547,
                start: 0,
                docs: [{ id: 'l_510', catch_line: 'doug put this here' }],
              },
            },
          ],
        },
              ...
              doclist: {
                ...
                docs: [
                  { id: 'l_11730', catch_line: 'Definitions.' },
                  {
                    id: 'l_22002',
                    catch_line:
                      '(Effective until October 1, 2012) Frequency of inspection; scope of inspection.',
                  },
                  {
                    id: 'l_3845',
                    catch_line:
                ...
                docs: [
                  {
                    id: 'l_5780',
                    catch_line:
                      'Approved plan required for issuance of grading, building, or other permits; security for performance.',
                  },
      ...
      debug: {
        ...
        explain: {
          l_11730: {
            ...
            details: [{ match: true, value: 1.0, description: 'queryNorm' }],
          },
          l_22002: {
            match: true,
            value: 1.0,
  ...
  describe('vars', () => {
    ...
    it('many custom defaults', async () => {
      ...
      var params = {
        q: [
          '#$keyword1## query #$keyword2## nothing #$keyword3|someDefault## #$keyword3|otherDefaults## #$keyword2##',
        ],
      };
      var exp = structuredClone(params);
    ...
    it('many custom defaults, others not customized', async () => {
      ...
      var params = {
        q: [
          '#$keyword1## query #$keyword2## nothing #$keyword3|someDefault## #$keyword3|otherDefaults## #$keyword3## #$keyword2##',
        ],
      };
      var exp = structuredClone(params);
      exp.q[0] = 'burrito query taco nothing someDefault otherDefaults  taco';
    ...
    it('super long query', async () => {
      var mockQT =
        'burrito taco nacho bbq turkey donkey michelin stream of consciousness taco bell cannot run away from me crazy muhahahaa peanut';
      var params = { q: ['#$keyword1## query #$keyword2## nothing #$keyword3##'] };
      var exp = structuredClone(params);
  ...
  describe('paging', () => {
    ...
    it('pages on page', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedPageParams))
        .respond(200, fullSolrResp);
  ...
  describe('explain other', () => {
    ...
    it('passes two solr queries one explains the other', async () => {
      ...
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, expectedExplOtherParams))
        .respond(200, mockSolrExplOtherResp);
      mockBackend
        .expectJSONP(urlContainsParams(mockSolrUrl, { q: ['title:doc1'] }))
    ...
    it('paginates for explain other searches', async () => {
      ...
      mockBackend
        .expectJSONP(
          urlContainsParams(mockSolrUrl, { q: ['title:doc1'], start: ['10'], rows: ['10'] }),
        )
        .respond(200, mockSolrResp);
