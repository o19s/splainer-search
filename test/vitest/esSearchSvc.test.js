...
describe('searchSvc: ElasticSearch', () => {
  ...
  describe('basic search', () => {
    describe('version 7+', () => {
      ...
      it('reports pretty printed errors for ES errors but HTTP success', async () => {
        var errorMsg = {
          hits: [],
          _shards: { failed: 1, failures: [{ foo: 'your query just plain stunk' }] },
        };
      ...
      it('sets the proper headers for auth', async () => {
        ...
        mockBackend
          .expectPOST(targetUrl, undefined, function (headers) {
  ...
  describe('highlights', () => {
    ...
    it('gets highlight snippet field values if returned', async () => {
      ...
      await searcher.search().then(function () {
        ...
        expect(docs[0].highlight('AU8pXbemwjf9yCj9Xh4e', 'title', '<b>', '</b>')).toEqual([
          '<b>Rambo</b>',
        ]);
      });
      mockBackend.verifyNoOutstandingExpectation();
  ...
  describe('paging', () => {
    ...
    function pagerValidator(expectedPagerParams) {
      return {
        test: function (data) {
          var parsed = JSON.parse(data);
          return (
            parsed.from === expectedPagerParams.from && parsed.size === expectedPagerParams.size
          );
        },
      };
    }
    ...
    it('pages on page', async () => {
      ...
      mockBackend
        .expectPOST(mockEsUrl, pagerValidator({ size: 10, from: 10 }))
        .respond(200, fullResponse);
      await nextSearcher.search();

      nextSearcher = nextSearcher.pager();
      mockBackend
        .expectPOST(mockEsUrl, pagerValidator({ size: 10, from: 20 }))
        .respond(200, fullResponse);
      await nextSearcher.search();
    ...
    it('accounts for custom rows count', async () => {
      ...
      var nextSearcher = searcher.pager();
      mockBackend
        .expectPOST(mockEsUrl, pagerValidator({ size: 20, from: 20 }))
        .respond(200, fullResponse);
      await nextSearcher.search();
  ...
  describe('failures', () => {
    ...
    it('reports failures', async () => {
      ...
      await searcher.search().then(
        ...
        function (msg) {
          expect(msg.searchError).toContain(
            "ElasticsearchIllegalArgumentException[field [cast] isn't a leaf field]",
          );
          errorCalled++;
        },
      );
    ...
    var expectedDocs = [
      {
        ...
        _id: 'l_1',
        _score: 5.0,
        fields: { field: ['1--field value'], field1: ['1--field1 value'] },
      },
      {
        _index: 'statedecoded',
        _type: 'law',
        _id: 'l_1',
        _score: 3.0,
        fields: { field: ['2--field value'], field1: ['2--field1 value'] },
      },
    ];
  ...
  describe('version', () => {
    ...
    it('defaults to version 5.0 and uses the "_source" params', async () => {
      ...
      mockBackend
        .expectPOST(mockEsUrl, function (postData) {
  ...
  describe('templated search', () => {
    ...
    it('returns docs, and removes _source and highlight query params', async () => {
      mockBackend
        .expectPOST(mockEsUrl + '/template', function (data) {
          var esQuery = JSON.parse(data);
          return (
  ...
  describe('templated and proxied search', () => {
    ...
    it('returns docs, and removes _source and highlight query params', async () => {
      mockBackend
        .expectPOST('http://myserver/api?url=' + mockEsUrl + '/template', function (data) {
          var esQuery = JSON.parse(data);
          return (
            esQuery.id === 'tmdb-title-search-template' &&
            esQuery.highlight === undefined &&
  ...
  describe('scripted fields', () => {
        ...
        hits: [
          {
            ...
            _id: 'l_1',
            _score: 5.0,
            _source: { vote_avg_times_two: [15.399999618530273] },
          },
          {
            _index: 'statedecoded',
            _type: 'law',
            _id: 'l_1',
            _score: 3.0,
            _source: { vote_avg_times_two: [10.800000190734863] },
          },
        ],
      },
    };
    ...
    beforeEach(() => {
      ...
      var scriptedParams = {
        ...
        script_fields: {
          vote_avg_times_two: {
            script: { lang: 'painless', source: "doc['vote_average'].value * 2" },
          },
        },
      };
      searcher = searchSvc.createSearcher(
  ...
  describe('rendering templates', () => {
    ...
    it('returns the rendered template showing the underlying query to be issued', async () => {
      mockBackend
        .expectPOST('http://localhost:9200/_render/template')
    ...
    it('rejects with formatted error on HTTP error', async () => {
      ...
      var errorCalled = 0;
      await searcher.renderTemplate().then(
        function () {
          errorCalled--;
        },
        function (msg) {
  ...
  describe('explainOther error handling', () => {
    ...
    it('handles search failure in explainOther gracefully', async () => {
      ...
      var called = 0;
      await searcher.explainOther(otherQuery, mockFieldSpec).catch(function () {
        called++;
      });
      mockBackend.verifyNoOutstandingExpectation();
      expect(called).toEqual(1);
