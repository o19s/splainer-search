...
As of splainer-search 3.0, JSONP is preserved via `<script>` tag injection in `httpClient.js`, matching the behavior Angular's `$http.jsonp()` provided. However, JSONP has significant downsides:
...
- **Legacy**: Modern Solr supports CORS headers natively. All other backends in splainer-search (Elasticsearch, OpenSearch, Algolia, Vectara, custom Search API, etc.) use GET/POST over HTTP with CORS as usual, not JSONP.
