...
export function explainSvcConstructor(baseExplainSvc, queryExplainSvc, simExplainSvc, utilsSvc) {
  ...
  var createExplain = function (explJson) {
    ...
    //this code captures the changed behavior
      ...
      if (details.length === 1) {
        ...
        //fall back to default behavior of detecting 'weight' in descriptions
        WeightExplain.prototype = base;
        return new WeightExplain(explJson);
      }
      ...
      description.startsWith('match on required clause') ||
      description.startsWith('match filter')
    ) {
      ...
      description.includes('constant score') &&
      description.includes('no function provided')
    ) {
      return IGNORED;
