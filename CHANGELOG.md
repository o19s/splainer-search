...
# 3.3.0 (2026-09-12)
- Algolia documents now support real highlighting: `snippet(docId, fieldName)` and `highlight(docId, fieldName, preText, postText)` read Algolia's own `_snippetResult`/`_highlightResult` response data (previously stubbed to always return `null`, silently falling back to plain unhighlighted text everywhere highlighting is displayed). Handles both scalar fields (a single `{value, matchLevel}` object) and array fields like a cast list (an array of those objects, one per entry) - see the README's Algolia section for details. `origin()` also now excludes `_highlightResult`/`_snippetResult`/`_rankingInfo` from a doc's origin output, matching how `esDocFactory.js` already excludes its own engine-specific response metadata. by @epugh.
- Algolia documents' `explain()` (previously always `{}`) now merges in the record's raw `_rankingInfo` (present when the query sets `getRankingInfo: true` - typo count, word/proximity/filter match info, Custom Ranking `userScore`, etc.) when available. `value`/`description`/`details`/`match` stay pinned to the same harmless stub every other engine without real explain data already returns, so `score()` and a host app's per-term score-breakdown chart are unaffected either way - only `explain().rawStr()` (or any other reader of `explain().asJson`) sees the extra fields, as one raw JSON blob. by @epugh.
- Fixed: `getHighlightSnippet` (`normalDocsSvc.js`, backing `subSnippets()`/`getHighlightedTitle()`) now unwraps a single-fragment `highlight()` result to a plain string. Every engine's `highlight()` always returns an array (an engine can return multiple highlighted fragments per field), but a host app's snippet display commonly only renders trusted HTML for a plain string, treating any array - including a 1-element one, the common case for a single match - as raw data to display as-is instead. Affects every engine, not just Algolia; a genuinely multi-fragment result (more than one array element) is unaffected and still returned as an array. by @epugh.
- **Breaking (minor):** `explain()`'s fallback for missing/empty explanation data no longer sets `description` to the literal string `'no explain for doc'` - it's now `''`. This library was making a host app's UI/copy decision on its behalf; a host app should instead detect "no explanation available" generically via `explain().children.length === 0` (true for this fallback, since it also has empty `details`) and decide its own wording/UI for that case. Any host app pattern-matching on the old literal string needs to switch to that check. by @epugh.

# 3.2.2 (2026-09-10)
- **Breaking:** `SettingsValidatorFactory` and `ResolverFactory` (and `wired.settingsValidatorFactory`) are removed. Each engine's own searcher now implements `fetchDocs(ids, fieldSpec, chunkSize)` and `validateUrl()` directly (inherited from the shared `SearcherFactory` base, with per-engine `_extractSourceDoc`/`_alwaysPresentFields` overrides where needed) instead of two separate factories re-deriving per-engine query/field-extraction logic that each engine's own factory already owns. Use `searchSvc.createValidator(settings)` in place of `new SettingsValidatorFactory(settings)` (same `.validateUrl()`/`.fields`/`.idFields` contract). `docResolverSvc.createResolver(ids, settings, chunkSize)` is unchanged - it's a thin compatibility wrapper now, but still returns the same `{docs, fetchDocs()}` shape.
- Solr now supports [JSON Query DSL](https://solr.apache.org/guide/solr/latest/query-guide/json-query-dsl.html) requests via `config.jsonQueryDsl: true`, for queries (nested `bool`/`edismax` clauses, etc.) that don't map onto classic `q=...&fq=...` query-string params. See the README's "Solr's JSON Query DSL" section for the full set of differences from classic mode (always POST, `params`-nested `debug`/`highlight`/`explainOther`, `limit`/`offset` paging, a `json=`-based `linkUrl`). https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Custom Search API supports a third `apiMethod: 'AUTO'` option (alongside `'GET'`/`'POST'`): sends a GET when the built URL fits within `config.maxGetUrlLength` (default 2000 characters, including the `config.proxyUrl` prefix if one is set), otherwise falls back to POST. See the README's "Automatic GET/POST switching" section. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: `config.escapeQuery`'s Lucene-syntax escaping was applied a second time on top of `hydrateSearchQuery`'s own default backslash/quote escaping for Solr, Elasticsearch/OpenSearch, and Custom Search API requests, corrupting queries containing `"` or `\`. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: JSONP and GET can't carry a request body, so a Solr `jsonQueryDsl: true` request used to silently send an empty request unless `config.apiMethod` was explicitly `'POST'` - it now always forces POST regardless of that setting. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: Solr's `explainOther()` set a classic-mode-only top-level key that Solr's JSON Request API rejects when `jsonQueryDsl` is enabled; it's now nested under `params` there instead. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: Custom Search API's `apiMethod: 'AUTO'` GET/POST length decision ignored `config.proxyUrl`'s length, so a proxied request that was actually too long for a GET could still be sent as one. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: Solr's `config.debug`/`config.highlight` were silent no-ops when `jsonQueryDsl` was enabled; they're now embedded under the JSON body's `params` key, matching classic mode's behavior. https://github.com/o19s/splainer-search/pull/163 by @epugh.
- Fixed: Solr's `searcher.linkUrl` (the "open in Solr" link) was a bare endpoint URL in `jsonQueryDsl` mode, losing the query entirely; it now embeds the full JSON body as Solr's own `json=` query parameter, verified to return identical results to the POST request. https://github.com/o19s/splainer-search/pull/163 by @epugh.
